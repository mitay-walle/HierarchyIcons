...
{
    #if UNITY_EDITOR
    private static readonly DebugToggle ASVolume = new DebugToggle("AudioSource Hierarchy Visualize");
    ...
    private static readonly Color AttentionColor = new Color(.7f, .3f, .3f, 0.2f);
    ...
    static void Init()
    {
        ...
        {
            if (!DeveloperOptions.current)
            {
                Debug.LogError("DeveloperOptions не найден!");
                return;
            }

            if (DeveloperOptions.current.Developer.HasFlag(eDeveloperFlags.Mitya))
            {
            ...
            else if (DeveloperOptions.current.Developer.HasFlag(eDeveloperFlags.Kostya))
            {
                EditorApplication.projectWindowItemOnGUI += ProjectOnGUI;
            }
...
//        }
    ...
    private static Texture2D GetTex(IHierarchyIconBehaviour obj)
    {
        ...
        if (!icons.ContainsKey(type)) icons.Add(type,AssetDatabase.LoadAssetAtPath<Texture2D>($"Assets/Gizmos/HierarchyIcons/{obj.EditorIconName}.tga"));
